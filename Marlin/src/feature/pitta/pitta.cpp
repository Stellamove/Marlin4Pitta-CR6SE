...
#define PITTA_MAIN_TICK HAL_timer_get_count(PITTA_TIMER_NUM) // 2000 per 1ms

HAL_PITTA_TIMER_ISR() {
  HAL_timer_isr_prologue(PITTA_TIMER_NUM);
  HAL_timer_isr_epilogue(PITTA_TIMER_NUM);
}
...
void PITTA::init()
{
  ...
  HAL_timer_start(PITTA_TIMER_NUM, 122);
  SET_INPUT_PULLDOWN(ONE_W_CMD_PIN);
}
#endif
...
#ifdef DEV_NCMD_PRINT
      ui.status_printf_P(0, PSTR("PE:%i CE:%i TB_L:%i"), int(wrong_packet_cnt), int(wrong_cmd_cnt), int(tb_len));
#else
      ui.status_printf_P(0, PSTR("F:%i N:%i TB_LEN:%i"), int(g_index + 1), int(change_turn_val), int(tb_len));
#endif 
#endif
  ...
  if (abs_issue_trigger_remain_tick > i_ABS_MOT_TRIGGER_CNT_DUTY)
  {
    ...
    if (b_change_done)
    {
      ...
      if (b_material_detected_from_pitta) {
        ...
        if (l_cnt>50) {//
          l_cnt = 0;
          // ui.status_printf_P(0, PSTR("hyst %i & jam %i"), int(mat_det_hyst_cnt), int(b_pitta_jammed));
          // pitta_ui_thermal_update(); 
          pitta_req_manage_heater_update();
        }
      }
        ...
        if (ll_cnt>100) {//
          ll_cnt = 0;
          // ui.status_printf_P(0, PSTR("hyst_n %i & jam %i"), int(mat_det_hyst_cnt), int(b_pitta_jammed));
          // pitta_ui_thermal_update(); 
          pitta_req_manage_heater_update();
        }        
        mat_det_hyst_cnt-=2;
...
// #endif
// #ifdef MODEL_E3V2_PRO32
//         ui.status_printf_P(0, PSTR("stp e%i & turn %i"), int(jam_expire_cnt), int(change_turn_val));
// #endif           
        // delay(10);
        // mExtruder_dir(b_mot_dir);
        // delay(10);
